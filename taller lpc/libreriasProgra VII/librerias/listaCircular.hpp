...
void ListaCircular<T>::liberarLista() {
    ...
    // 1. Rompemos la circularidad para tratarla como una lista normal.
    //    Esto evita un bucle infinito al recorrerla para borrar.
    this->tail->setSiguiente(NULL);

    Nodo<T>* actual = this->head;
    Nodo<T>* siguiente = NULL;

    // 2. Recorremos la lista ahora lineal y liberamos cada nodo.
    while (actual != NULL) {
    ...
    // 3. Reiniciamos los punteros y la longitud de la lista.
    this->head = NULL;
    this->tail = NULL;
    this->longitud = 0;
}
...
//Eliminar un nodo en la posicion pos en una lista circular
template<typename T>
...
template<typename T>
int ListaCircular<T>::buscar(T element) {
    Nodo<T> *actual = head;
    int i = 0;
    while(!this->esVacia() && actual->getElemento() != element){
        i++;
        actual = actual->getSiguiente();
    }
    if(actual->getElemento() == element) return i;
    return -13;
} 
...
const T& ListaCircular<T>::consultar(int pos) const {
    ...
    if (pos <= 0) return head->getElemento();
...
void ListaCircular<T>::rightShift(int shift){
    ...
    if (longitud <= 1 || shift <= 0) {
        return;
    }
    int realShift = shift % longitud;
    if (realShift == 0) {
        return;
    }
    int movements = longitud - realShift -1;
    Nodo<T>* nuevoTail = head;
    for (int i = 0; i < movements; i++) {
        nuevoTail = nuevoTail->getSiguiente();
    }
    head = nuevoTail->getSiguiente();
    tail = nuevoTail;
}
template<typename T>
